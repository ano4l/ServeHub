...
export function AppTabs() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-[70] border-t border-stone-200 bg-white/95 px-3 py-2 backdrop-blur supports-[backdrop-filter]:bg-white/85">
      ...
      {TABS.map((tab) => {
        ...
        return (
          <Link
            ...
            href={tab.href}
            className={cn(
              "inline-flex h-11 min-w-[74px] shrink-0 items-center justify-center gap-2 rounded-xl border px-3 text-sm font-medium transition",
              active
